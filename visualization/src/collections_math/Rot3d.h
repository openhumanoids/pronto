...
// BEGIN REMOVE FROM RELEASE
  static Eigen::MatrixXd exmap_dquat(/*const Eigen::Quaterniond& quat*/) {
    // note: we evaluate the derivative at delta=0 - the linearization
    // point is given by the quaternion, but doesn't play a role here either
    Eigen::MatrixXd dq_dd = Eigen::MatrixXd::Zero(4,3);
    dq_dd(1,0) = 0.5;
    dq_dd(2,1) = 0.5;
    dq_dd(3,2) = 0.5;
    return dq_dd;
