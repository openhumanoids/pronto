...
void processThread(App& app) { 
  ...
  while (1==1) {
    ...
    if ( app.do_convert_cloud_ ) {
      ...
      std::stringstream s1;
      s1 <<  "/media/mfallon/data/borg-lcm/pcd/" << app.accu_->getFinishedTime() <<  ".pcd" ;
      std::cout << "Saving original point cloud to: " << s1.str() << "\n";
      //pc_vis_->writePCD(s1.str(), *cloud);
      app.accu_->writePCD(s1.str(), cloud);
      ...
      std::cout << "Restart mapping\n";
      app.do_accum_ = true;
      app.accu_->clearCloud();
